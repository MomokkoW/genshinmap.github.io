...
// const filterContext = require.context('images/icons/filter', false, /\.png/);
const _MapControlSummaryFeature = ({ featureKey, completedCount, displayed }) => {
  const classes = useStyles({ bgImage: require('images/controls/filter_border.png?webp') });
  ...
  return (
    <Box className={clsx(classes.featureRoot)}>
      <Box className={classes.iconBorder}>
        {/*<Img className={classes.icon} src={filterContext(`${mapFeature.icons.filter}.png`)} />*/}
      </Box>
