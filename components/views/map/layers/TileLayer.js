...
const tileContext = require.context('images/tiles', false, /\.png/);
...
const LAdvancedTileLayer = L.TileLayer.extend({
  ...
  getTileUrl(coords) {
    const { x, y, z } = coords;
    const image = tileContext(`Map_${z}_${x}_${y}.png`);
    console.log(tileContext.keys());
    console.log(image);
    return image;
  },
});
...
const TileLayer = () => {
  ...
  // The URL is built by the tile layer. Sorry!
