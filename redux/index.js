...
const ENABLE_REDUX_DEBUG = isDev();

const createMiddleware = () => {
  if (ENABLE_REDUX_DEBUG && typeof window !== 'undefined') {
    // Add in Redux Dev Tools support.
    return (window?.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ || compose)(
      applyMiddleware(rootMiddleware)
    );
  } else {
    return compose(applyMiddleware(rootMiddleware));
  }
};
